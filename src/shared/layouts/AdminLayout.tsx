...
import { 
  LayoutDashboard, Users, Trophy, MapPin, Settings, LogOut, 
  ChevronRight, Menu, X, Bell, Search, HelpCircle
} from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { path: '/admin/events', label: 'Events', icon: <Trophy size={20} /> },
    { path: '/admin/venues', label: 'Venues', icon: <MapPin size={20} /> },
      ...
      <header className="bg-white border-b border-slate-200 flex items-center justify-between px-4 py-2 lg:py-3">
        <div className="flex items-center">
          ...
          <Link to="/admin/dashboard" className="flex items-center">
            <div className="bg-purple-600 text-white w-8 h-8 rounded-md flex items-center justify-center font-bold mr-2">
              M
            </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="relative hidden lg:block">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="pl-9 pr-4 py-1.5 border border-slate-300 rounded-md text-sm w-64 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            />
          </div>
              ...
              <ul className="space-y-1">
                {navItems.map((item) => (
                  <li key={item.path}>
                    ...
                    >
                      <span className={`${isActive(item.path) ? 'text-purple-600' : 'text-slate-500'} mr-3`}>
                        {item.icon}
                      </span>
